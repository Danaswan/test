...
class ProfitStatement extends AbsServer
{
	...
	protected $colMap = [
		'amazon' => [
			'title' => [
				...
				'AF' => ['title' => '物流商', 'width' => 30],
				'AG' => ['title' => '转运费', 'width' => 30],
			],
			'data' => [
				...
				'trans_shipping_fee' => ['col' => 'AG', 'type' => 'str'],
			]
		],
		'wish' => [
			'title' => [
		...
		'aliExpress' => [
			'title' => [
				...
				'Z' => ['title' => '物流商', 'width' => 30],
				'AA' => ['title' => '转运费', 'width' => 30],
		...
		'ebay' => [
			'title' => [
				...
				'AA' => ['title' => '物流商', 'width' => 30],
				'AB' => ['title' => '转运费', 'width' => 30],
			],
			'data' => [
				'order_number' => ['col' => 'A', 'type' => 'str'],
				'order_type' => ['col' => 'B', 'type' => 'str'],
		...
		'joom' => [
			'title' => [
				...
				'AB' => ['title' => '物流商', 'width' => 10],
				'AC' => ['title' => '转运费', 'width' => 10],
			],
			'data' => [
				'order_number' => ['col' => 'A', 'type' => 'str'],
				'channel_order_number' => ['col' => 'B', 'type' => 'str'],
		...
		'jumia' => [
			'title' => [
				...
				'AF' => ['title' => '物流商', 'width' => 10],
				'AG' => ['title' => '转运费', 'width' => 10],
			],
			'data' => [
				'order_number' => ['col' => 'A', 'type' => 'str'],
				'channel_order_number' => ['col' => 'B', 'type' => 'str'],
	...
	private function initFieldValue($channel_id, $data)
	{
		...
		switch ($channel_id) {
			case 1:
				...
				break;
			case 2:
				if (isset($data['is_fba']) && $data['is_fba'] == 1) {
	...
	public function search($params)
	{
		...
		$params['is_fba'] = param($params, 'is_fba', 0);
		...
		try {
			...
			switch (($params['channel_id'])) {
				case 1:
					...
					break;
				case 2:
					if (isset($params['is_fba']) && $params['is_fba'] == 1) {
	...
	protected function getPlatformOrderProfit($channelId, $recordData)
	{
		...
		switch ($channelId) {
			case 2: //亚马逊
				if (isset($_data['is_fba']) && $recordData['is_fba'] == 1) {
...
//            $_data['channel_order_number'] = $_data['channel_order_number'];//避免科学计数法
			...
			switch ($_data['channel_id']) {
				case 1: //ebay
					...
					break;
				case 2: //amazon
					if (isset($_data['is_fba']) && $_data['is_fba'] == 1) {
						...
						$_data['receipt_fee'] = bcmul($_data['order_amount_CNY'] - $_data['channel_cost_CNY'], $this->PCardRate['fba'], 4);
	...
	{
		if ($params['channel_id'] == ChannelAccountConst::channel_amazon && ($params['is_fba']) == 1) {
	...
	protected function getSearchCondition(&$params)
	{
		...
		$orderId = $this->getParameter($params, 'order_id', '');
		if ($orderId) {
	...
	{
		if ($params['channel_id'] == ChannelAccountConst::channel_amazon && ($params['is_fba'] == 1)) {
			return $this->fbaSearch($params, $page = 1, $pageSize = 10, $timeField = '', $timeSort = 'ASC');
		}
